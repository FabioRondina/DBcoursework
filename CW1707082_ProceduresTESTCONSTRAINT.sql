...
if they do not already exist). The patient’s data should
 ...
 --Complete solution
	/* -this solution maintain the same two separate tables to archive the data, in my opinion this will take less space and
	   ...
	   -I added the same constraints of the original tables to keep the same structure. I had to build it with multiple IF statements since
		the syntax i found online (IF THEN ELSE END IF) seems not working correctly here 
		*/
	
DROP PROCEDURE ArchivePatient
...
FROM Patient 
WHERE pat_id = @patid;
--add same constraint as the original to the archive table
IF OBJECT_ID('Appointment_Archive','U') IS NULL
ALTER TABLE Patient_Archive ADD 
CONSTRAINT PK_PAtient_Archive PRIMARY KEY (pat_id);
